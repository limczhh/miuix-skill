...
- `Scaffold` can host a `floatingToolbar` with a `ToolbarPosition`; this slot predates 0.9.4, while the current stable layout keeps the `Snackbar` above a bottom-positioned toolbar.
...
- In stable `0.9.4`, `NavigationRail` has separate fixed-layout and non-null expandable-state overloads; `state = null` is no longer a valid compatibility call. `NavigationBarItem` and `FloatingNavigationBarItem` accept a shared `NavigationBarItemColors` value before `badge`.
- Stable `miuix-nav` provides `LocalNavTransitionScope` inside each entry, adds the composition-safe `NavTransitionScope.isRunning` signal, and passes the parent saved-state creation context into entry ViewModels so `SavedStateHandle`/Hilt integrations can work without custom owner plumbing.
- Stable `miuix-ui` includes `PagerGestureUtils` for `HorizontalPager`/nested-list conflicts, including `pagerGestureOverride`, explicit interception modes, and spring page navigation. `TabRow` now contains horizontal nested scroll/overscroll instead of forwarding it to a parent pager.
...
- [v0.9.3 → v0.9.4 migration notes](references/release-v0.9.4.md)
...
1. Confirm that the target project's Miuix dependency is `0.9.4` before using a version-sensitive example: inspect its Gradle files, version catalog, dependency lock, or build output and identify the platform/source set. This Skill's current API guidance is for stable `0.9.4`; if the target uses another version, state the mismatch instead of presenting these examples as version-correct. For a `v0.9.3 → v0.9.4` upgrade, use the single migration reference.
...
6. Load evidence at the required depth. For the current catalog, use the pinned web-source paths in [Source verification](references/source-verification.md): read the relevant Markdown doc, demo, Example code, and source from the stable `v0.9.4` snapshot at commit `39c40f99`. If the user is explicitly migrating from `v0.9.3`, use the single migration reference and its comparison evidence.
...
9. Verify exact APIs against the pinned stable snapshot in [Source verification](references/source-verification.md). Compile code changes and validate visual changes with a render, screenshot, preview, emulator, or device when available.
...
- Include dependency lines only when setup is part of the task or the target project is missing them. The dependency examples and catalog in this Skill target stable `0.9.4`; do not silently apply them to another Miuix version.
...
- When the stable snapshot has no public Miuix component for the requested interaction, compose it first from Compose Foundation/Layout/Animation primitives under `MiuixTheme`, reuse Miuix semantic tokens and nearby component Defaults, and keep the custom wrapper application-owned. Mix Material or Material 3 only when the target project intentionally does so or the user explicitly requests it. Disclose the custom behavior and verify its semantics, disabled/error states, and platform behavior.
- Verify `BreadcrumbBar`, the existing `Scaffold.floatingToolbar` slot and its stable snackbar spacing, `InputField.color` (not `SearchBar.color`), `OverlayDialog`/`WindowDialog` `maxWidth`/`largeScreen`/`cornerRadius`, `WindowNavigationEventScope`, `PullToRefresh` progress APIs, `RadioButtonPreference.colors`, `NavigationRail`'s two stable overloads, `NavigationBarItem.colors`, and `ProgressiveBlur` before giving migration-sensitive examples.
- For `miuix-nav`, verify `NavKey`, `rememberNavBackStack`, `NavDisplay`'s `entry<T> { ... }` DSL, `NavTransitions`, `NavDisplayEffects`, `LocalNavTransitionScope`, and `NavTransitionScope.isRunning` in the stable source. Do not fabricate `androidx.navigation3` scene APIs or a built-in result channel.
- For pager conflicts, verify the stable `PagerGestureUtils` configuration (`userScrollEnabled = false` plus `PagerGestureNestedScrollConnection` for cross-axis interception) instead of inventing a generic nested-scroll workaround.
