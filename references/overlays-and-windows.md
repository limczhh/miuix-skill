...
- At the current stable snapshot, dialog, bottom-sheet, and list-popup layouts route system back dismissal to `onDismissRequest`; Window variants also use their platform window host where applicable. The caller must update the controlled `show` state. Always provide the callback for a dismissible popup.
...
- Stable 0.9.4 provides `WindowNavigationEventScope` in `miuix-ui/.../utils/`. Window dialogs, sheets, and list/cascading popups apply it automatically so `NavigationBackHandler`/predictive-back handlers resolve against the focused platform window. Wrap custom separate-window hosts that contain the same handlers.
...
In the current stable snapshot, `OverlayDialog` and `WindowDialog` both accept:
...
The public `maxWidth` parameter exposes `DialogDefaults.MaxWidth` as the default limit for both dialog wrappers. Use `DialogDefaults.MaxWidth` and `DialogDefaults.CornerRadius` when a product does not have a deliberate design value. Keep the dialog's content and dismissal state controlled by the caller; these parameters do not remove the need for a `Scaffold` ancestor for `OverlayDialog`.
