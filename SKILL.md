---
name: miuix
description: "Miuix (HyperOS) Compose UI component library expert. Use this skill whenever the user asks to build UI with Miuix components, mentions Xiaomi/HyperOS design style, or references any Miuix component name (NavigationBar, SwitchPreference, Scaffold, OverlayDialog, ArrowPreference, SliderPreference, Tooltip, Badge, TopAppBar, SearchBar, etc.). Also trigger when the user describes a UI scenario that maps to Miuix components: settings screens, preference pages, dialogs, bottom sheets, dropdown menus, popup menus, search bars, tab navigation, floating toolbars, or any HyperOS-style UI. In the miuix-main project, Miuix is the default UI toolkit — use this skill for ALL UI work here UNLESS the user explicitly asks for Material 3, MD3, or another design system. Covers: project setup (Gradle dependencies), theme configuration (ThemeController, dark mode, dynamic colors), component selection, API reference, code examples, and migration between versions. Do NOT use this skill when the user wants standard Compose Material components, Jetpack Compose Material 3, or explicitly asks for a non-Miuix design system."
version: "0.9.2"
---
